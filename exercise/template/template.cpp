...
struct CardGame
{
    ...
    {
        if(creature1 < 0 || creature1 >= creatures.size() || creature2 < 0 || creature2 >= creatures.size())
            return -1;
        auto& c1 = creatures[creature1];
        auto& c2 = creatures[creature2];

        hit(c1, c2);
        hit(c2, c1);
        if(c1.health >0 && c2.health <=0)
            return creature1;
        else if(c1.health <= 0 && c2.health>0)
            return creature2;
        else
            return -1;
    }
